...
export default function AdminBoardClient() {
  ...
  async function accept(id:string){
    ...
    await fetch('/api/adminboard/accept',{
      ...
      body:JSON.stringify({id,date:`${date}T${time}`,place})
    })
    load()
  }
  ...
  async function remove(id:string){
    if(!confirm("Bewerbung in Papierkorb verschieben?")) return
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end mb-6 gap-4">
        <label className="text-gray-400">Sortieren nach:</label>
        <select
          value={sortBy}
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-7">
        {sortedApplications.map(app => (
          <motion.div
            ...
            className="bg-gray-800/80 backdrop-blur-md rounded-2xl p-6 border border-gray-700 shadow-xl"
          >
            ...
            <div className="flex gap-2 mt-2">
              <button onClick={()=>accept(app.id)} className="flex-1 bg-green-600 hover:bg-green-700 transition px-3 py-2 rounded-lg text-sm">Annehmen</button>
              <button onClick={()=>reject(app.id)} className="flex-1 bg-yellow-600 hover:bg-yellow-700 transition px-3 py-2 rounded-lg text-sm">Ablehnen</button>
              <button onClick={()=>remove(app.id)} className="flex-1 bg-red-600 hover:bg-red-700 transition px-3 py-2 rounded-lg text-sm">Löschen</button>
            </div>
          </motion.div>
        ))}
      </div>
