'use client'
import { ClipboardList, Trash2, ScrollText, Lock, Newspaper, Inbox, Wrench } from 'lucide-react'
...
export default function Sidebar({ setView, view, applicationCount, session }: SidebarProps) {
  ...
  const isOwner = session?.user?.id === CRYPTIX_ID
  ...
  return (
    <div className="w-64 bg-black/40 backdrop-blur-xl border-r border-gray-800/60 p-5 flex flex-col gap-1">
      ...
      <p className="text-xs text-gray-600 px-1 mb-1">Verwaltung</p>
      {/* Wartungsarbeiten – alle sehen, nur Owner bearbeitet */}
      <button
        onClick={() => setView('maintenance')}
        className={`${btnBase} ${view === 'maintenance' ? btnActive : btnInactive}`}
      >
        <Wrench size={16} />
        Wartungsarbeiten
        {!isOwner && (
