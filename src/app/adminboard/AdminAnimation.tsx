...
export default function AdminAnimation({ onFinish }: AdminAnimationProps) {
  ...
  useEffect(() => {
    ...
    for (let y = 0; y < height; y += 3) {
      for (let x = 0; x < width; x += 3) {
        const i = (y * width + x) * 4
        if (img[i + 3] > 128) {
          temp.push(new THREE.Vector3((x - width / 2) / 40, -(y - height / 2) / 40, 0))
        }
      }
    }
  ...
  return (
    <div className="w-full h-screen bg-black relative overflow-hidden">
      ...
      <div
        className={`absolute top-20 w-full text-center text-white text-4xl font-bold transition-opacity duration-3000 ease-in-out ${
