...
export default function AdminBoard() {
  ...
  useEffect(() => {
    async function checkAdmin() {
      try {
        // Discord-Token aus Cookie oder LocalStorage (je nachdem wie du speicherst)
        const token = document.cookie.match(/discord_token=([^;]+)/)?.[1];
        ...
        if (!data.allowed) {
          // User ist kein Admin → redirect zu / mit query
          router.replace('/?error=admin');
          return;
        }

        // Admin erlaubt → Bewerbungen laden
        const appsRes = await fetch('/api/adminboard');
        const appsData = await appsRes.json();
        setApplications(appsData.applications || []);
      } catch (err) {
        console.error(err);
        router.replace('/?error=admin'); // sicherheitshalber redirect
      } finally {
        setLoading(false);
      }
    }
